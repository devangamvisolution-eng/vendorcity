...
class ServiceController extends Controller
{
    ...
    public function get_enquiry_form_fields(Request $request)
    {
        ...
        // Combine all tags (form_fields and form_fields_two)
        $form_fields_service = [];
        $form_fields_service_two = [];

        if ($form_field_data_ser) {
            $form_fields_service = $form_field_data_ser->form_fields ? explode(',', $form_field_data_ser->form_fields) : [];
            if (isset($form_field_data_ser->form_fields_two)) {
                $form_fields_service_two = $form_field_data_ser->form_fields_two ? explode(',', $form_field_data_ser->form_fields_two) : [];
            }
        }
        ...
        $all_tags = array_filter(array_unique(array_merge(
            $form_fields_service,
            $form_fields_subservice,
            $form_fields_service_two,
            $form_fields_subservice_two
        )));

        $fields = [];
        if (!empty($all_tags)) {
            $result = DB::table('form_fileds')->whereIn('id', $all_tags)->orderBy('set_order')->get();
