...
class Packagecontroller extends Controller
{
    ...
    public function getCleanersByCity(Request $request)
    {
        ...
        <li class="splide__slide text-center">
            <div class="cleaners-div" onclick="cleaner_data(\'' . $data->id . '\', \'' . $name . '\')">
                <div class="items">
                    ...
                    <div class="cleaner-image">
                        <img src="' . $image . '" class="cleaners-image-style">
                    </div>
    ...
    function painting_time_check(Request $request)
    {
        ...
        foreach ($timeslots as $slot) {
            ...
            if ($price > 0) {
                $html .= '<span>+ <p class="currency_dhiram"></p> ' . $price . '</span>';
            }
    ...
    public function package_inquiry(Request $request)
    {
        ...
        if ($request->subservice_id == 23 || $request->subservice_id == 26 || $request->subservice_id == 53) { //apartment moving,villa moving,office moving
    ...
    if (!empty($vendor_data)) {
        ...
        $vendor_att_email = [];
        $vendor_data_attr = DB::table('vendors_attribute')
        ...
        $form_fields_data = DB::table('more_formfields_details')
            ->where('package_inquiry_id', $packageEnquiryFormId)
            ->get()
            ->toArray();
        ...
        if ($vendor_data && $vendor_data->is_active == 0) {
            ...
            foreach ($form_fields_data as $key => $values) {
                ...
                if ($values->formfield_value != '') {
                    $field_array[$key] = array(
                        'name' => $form_fields->lable_name,
                        'value' => $values->formfield_value,
                        'id' => $form_fields->id,
                        'mail_send' => $form_fields->mail_send
                    );
                }
            }
            ...
            // your same email HTML design (unchanged)
            ...
            // ✅ Send mail asynchronously after response
            dispatch(function () use ($vendor_data, $subject, $ccRecipients, $html, $packageEnquiryFormId) {
                try {
                    Mail::send([], [], function ($message) use ($html, $vendor_data, $subject, $ccRecipients) {
                    ...
                    // WhatsApp alert
                    if ($vendor_data->country_code != '' && $vendor_data->mobile != '') {
                        $this->whatsappmsg_tmp_new_lead_alert($vendor_data,$packageEnquiryFormId);
                    }

                } catch (\Exception $e) {
                    \Log::error('Vendor mail failed (' . $vendor_data->email . '): ' . $e->getMessage());
                }
            })->afterResponse();
        }
    }
...
<div class="surcharge-badge-timeslot items">
    ' . ($price > 0
                ? '<span>+ <p class="currency_dhiram"></p> ' . $price . '</span>'
                : ''
            ) . '
