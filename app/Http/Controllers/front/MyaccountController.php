...
class MyaccountController extends Controller
{
     ...
     */
    /**
     * @ARCHITECTURE_NOTE Subscription State Machine
     *
     * Subscription States:
     * - Pending: Purchased but first booking not started
     * - Active: Subscription operating normally
     * - Paused: Temporarily frozen
     * - Renewal Upcoming: Approaching next billing
     * - Payment Failed: Renewal transaction unsuccessful
     * - Grace Period: Payment being retried
     * - Ending: Auto-renewal disabled
     * - Cancelled: Subscription terminated
     * - Completed: Fixed/non-renewing package completed
     * - Suspended: Admin/system suspension
     *
     * Visit Statuses (Must be tracked separately from Subscription status):
    ...
    public function subscription_detail($id, Request $request)
    {
        $userdata = Session::get('user');
...
if (empty($userdata)) {
    ...
    function view_receipts($id, Request $request)
    {
        ...
        if ($request->get('download') == 'pdf') {
            ...
            $fileName = $query->format_order_id . '.pdf';
            // echo "<pre>";
            // print_r($fileName);
            // echo "</pre>";
            // exit;
