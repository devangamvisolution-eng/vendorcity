...
class Homecontroller extends Controller
{
    ...
    //    echo "<pre>";print_r($response);exit;
        ...
        $formattedCity = ucwords(str_replace('-', ' ', session('search_city_name')));
     ...
     // $data['service']=DB::table('services')->orderBy('set_order')->get();
        ...
        $data['sub_service']=DB::table('subservices')->whereRaw("FIND_IN_SET(?, city)", [$cityId])->where('is_active', 0)->orderBy('set_order','ASC')->get();
        $data['city']=DB::table('cities')->where('country',22)->get();
...
//echo"<pre>";print_r($response);echo"</pre>";exit;
    ...
    echo"<pre>main";print_r($response);echo"</pre>";
    ...
    if(isset($vendors_attribute) && count($vendors_attribute) > 0){
        ...
        foreach($vendors_attribute as  $vendorAtt){
            ...
            if(isset($vendorAtt->country_code) && isset($vendorAtt->telephone)){
                    ...
                    $responseatt = curl_exec($curl);

                    curl_close($curl);

                    $responseatt = json_decode($responseatt, true);

                    echo"<pre>att";print_r($responseatt);echo"</pre>";
                }
        }
