...
class WoodenFloorLeadsController extends Controller
{
    public function index()
    {   
        $vendor_id = auth::user()->id;
      
        $data['all_data'] = DB::table('wooden_floor_enquiry')->where('vendor_id', 'LIKE', '%'.$vendor_id.'%')->orderBy('id', 'desc')->get();

        //echo"<pre>";print_r($data['all_data']);echo"</pre>";exit;       
        //echo"<pre>";print_r($data);echo"</pre>";exit;       
       return view('admin.vendor_wooden_floor_inquiry',$data);
    }

    function wooden_floor_assign_vendor(){
        
        $inquiry_id = $_POST['inquiry_id'];
		
		$wooden_floor_enquiry_data = DB::table('wooden_floor_enquiry')
            ->where('id', $inquiry_id)
            ->first();
        
        $currentDate = now();
			// echo "<pre>";print_r($wooden_floor_enquiry_data);echo"</pre>";exit;
        $html = '<label>Select a Vendor</label>';
        $html .= '<select id="wooden_vendor_id" name="wooden_vendor_id[]" class="form-control vendor-mul-drop select2" multiple="multiple">';
                
                $html .= "<option value=''>Select Vendor</option>";

        $vendor_data = DB::table('users')
                        ->join('subscription', 'subscription.vendor_id', '=', 'users.id')
                        ->whereRaw("FIND_IN_SET(?, users.serviceList)", [$wooden_floor_enquiry_data->service_id])
                        ->where('users.vendor', 1)
                        ->where('users.is_active', 0)
                        ->where('users.wallet_amount', '>', 0)
                        ->where('subscription.services', $wooden_floor_enquiry_data->service_id)
                        ->where('subscription.sub_service', $wooden_floor_enquiry_data->subservice_id)
                        ->where('subscription.is_deleted', 0)
                        ->select('users.*', 'subscription.id as subscription_id') // You can select fields you want
                        ->orderBy('subscription.id', 'DESC')
                        ->groupBy('users.id') // optional if multiple subscriptions exist for same vendor
                        ->get()
                        ->toArray();
                            
                foreach($vendor_data as $vendor_data_new){

                    if(in_array($vendor_data_new->id,explode(",",$wooden_floor_enquiry_data->vendor_id))){
                        $selected = "selected";
                    } else {
                        $selected = "";
                    }
    ...
    function wooden_floor_email_vendor(){
        ...
        $html .= '<select id="wooden_vendor_id" name="wooden_vendor_id[]" class="form-control vendor-mul-drop select2" multiple="multiple">';
                ...
                $vendor_data = DB::table('users')
                                ->whereRaw("FIND_IN_SET(?, serviceList)", [$wooden_floor_enquiry_data->service_id])
                                ->where('vendor', 1)
                                ->where('is_active', 0)
                ...
                foreach($vendor_data as $vendor_data_new){
                    ...
                    if(in_array($vendor_data_new->id,explode(",",$wooden_floor_enquiry_data->vendor_id_for_email))){
                        $selected = "selected";
                    } else {
                        $selected = "";
                    }
    ...
    function wooden_vendor_form(){


        $vendorId = implode(",",$_POST['wooden_vendor_id']);
		...
		$wooden_data = DB::table('wooden_floor_enquiry')
                                ->where('id', $inquiry_id)->first();

        // echo"<pre>";print_r($wooden_data);echo"</pre>";exit;
       ...
       // echo "<pre>";print_r(!empty(array_intersect((array) $painting_data->vendor_id, $arrVendor)));echo "</pre>";exit;
        ...
        $service_id = $wooden_data->service_id;
        $subservice_id = $wooden_data->subservice_id;
		...
		$subscriptionCharge = DB::table('subscription_subservice_attribute')->select('*')
			...
			{   
                ...
                // echo"<pre>";print_r($checkVendor);echo"</pre>";exit;
				if(empty($checkVendor)){

				$vendor_name = DB::table('users')->where('vendor', 1)->where('id',$vendordata->id)->value('name');

				$request_date = $wooden_data->enquiry_month.' '.$wooden_data->enquiry_date.', '.$wooden_data->enquiry_year;
				...
				$vendor_html .='<!doctype html>
							...
							<div class="email_wrapper" style="width:100%;margin-top: 18px;font-size: 16px;">
									  ...
									  $vendor_html .='<p><strong>Enquiry Details:</strong><br>';
										$vendor_html .='<ul>
															<li><strong>Service: </strong> '.\Helper::servicename($wooden_data->service_id).'</li>
															<li><strong>Property Type: </strong> '.$wooden_data->property_type.'</li>
															<li><strong>Customer Name: </strong> '.$wooden_data->name.'</li>
															 <li><strong>Customer Email: </strong> '.$wooden_data->email.'</li>
															<li><strong>Customer Contact: </strong> '.$wooden_data->mobile.'</li>
					...
					$subject = "New ".\Helper::servicename($wooden_data->service_id)." Enquiry Assigned on VendorsCity | Enquiry No. ".$wooden_data->inquiry_id." ";
					$to = $vendordata->email;
                    ...
                    DB::table('wooden_floor_enquiry')
                        ->where('id', $inquiry_id)
                        ->update(['vendor_id' => $vendorId]);
                    
					$walletAmountDeduct = $this->wooden_wallet_amount_deduct($vendor_id,$service_id,$subservice_id,$inquiry_id);
       ...
       //echo "<pre>";print_r($emptyWalletVendor);echo"</pre>";exit;
        ...
        return redirect()->route('wooden-floor-enquiry')->with('success',$successMessage)->with('error',$errorMessage);
    }

    function wooden_wallet_amount_deduct($vendor_id,$service_id,$subservice_id,$inquiry_id)
    {  
        $vendor_data = DB::table('users')->where('id',$vendor_id)->first();
        $wooden_data = DB::table('wooden_floor_enquiry')->where('id', $inquiry_id)->first();
    ...
    //    echo"<pre>";print_r($vendor_package_no_of_inquiry);echo"</pre>";exit;
       if($vendor_package_no_of_inquiry > $vendor_accept_inquiry_data->total_no_of_inquiry){
        ...
        if($redirect_type == 'success'){
                ...
                <div class="wrapper" style="width: 100%;max-width:500px;margin:auto;
                    ...
                    <table id="table_new" style="width: 100%;border: 1px solid black;
                        ...
                        <tr>
                            ...
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;border-collapse: collapse;"> '.$wooden_data->name.'</td>
                        </tr>
                        <tr>
                            <td class="custome_td" style="background-color: #0040E6;
                            ...
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;border-collapse: collapse;"> '.$wooden_data->email.'</td>
                        </tr>
                        <tr>
                            <td class="custome_td" style="background-color: #0040E6;
                        ...
                        border-collapse: collapse;"> '.$wooden_data->mobile.'</td>
                        </tr>
						<tr>
                            <td class="custome_td" style="background-color: #0040E6;
                            color: #fff;padding: 10px 60px 10px 12px;border-bottom-color: #fff !important;border: 1px solid black;
                        border-collapse: collapse;">Date:</td>
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;
                        border-collapse: collapse;"> '.$wooden_data->enquiry_month.' '.$wooden_data->enquiry_date.', '.$wooden_data->enquiry_year.'</td>
                        </tr>
						<tr>
                            <td class="custome_td" style="background-color: #0040E6;
                            color: #fff;padding: 10px 60px 10px 12px;border-bottom-color: #fff !important;border: 1px solid black;
                        border-collapse: collapse;">Time:</td>
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;
                        border-collapse: collapse;"> '.\Helper::timeslotname(strval($wooden_data->time_slot)).'</td>
                        </tr>
						<tr>';
                           
                        $html.='<td class="custome_td" style="background-color: #0040E6;
                            color: #fff;padding: 10px 60px 10px 12px;border-bottom-color: #fff !important;border: 1px solid black;
                        border-collapse: collapse;">Property Type:</td>
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;
                        border-collapse: collapse;"> '.$wooden_data->property_type.'</td>
                        </tr>';

                        $html.='<td class="custome_td" style="background-color: #0040E6;
                            color: #fff;padding: 10px 60px 10px 12px;border-bottom-color: #fff !important;border: 1px solid black;
                        border-collapse: collapse;">Area of Floor:</td>
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;
                        border-collapse: collapse;"> '.$wooden_data->area_of_floor.'</td>
                        </tr>';
			...
			$subject = "VendorsCity Wooden Floor Polishing  Lead for ".$wooden_data->property_type."! Here Are the Details";
        ...
        $customer_html_new .='<!doctype html>
                ...
                <div class="email_wrapper" style="width:100%;margin-top: 18px;font-size: 16px;">
                   ...
                   <p>We hope this email finds you well.</p>';
                    ...
                    $subject_type = "Survey Request for Your " . \Helper::servicename($wooden_data->service_id) ." Request on VendorsCity!";
                  $customer_html_new .='<p>'.$text1 .'<br><br>';

   
                $customer_html_new .='<span style="margin-bottom: 15px;display: inline-block;
                "><strong>Service Requested: </strong>' .  \Helper::servicename($wooden_data->service_id) .'</span><br>'; 
               ...
               <div class="email_footer" style="width:100%;margin-top: 20px;">
                            ...
                            float: left;">
                                ...
                                <a href="'.url("/contact").'"  style="width: 100%;color: #555;display: inline-block;">Contact Us</a>
                                    ...
                                    <p style="margin:0;">This message was mailed to '.$wooden_data->email.' as part of you account registered with us on VendorsCity</p>
                                </div>
                            </div>
                     </div>
                 </div>
...
function wooden_wallet_vendor(){
    $vendor_id = implode(",",$_POST['wooden_vendor_id']);
...
<div class="email_wrapper" style="width:100%;margin-top: 18px;font-size: 16px;">
        ...
        return redirect()->route('wooden-floor-enquiry')->with('success','The E-mail to the vendor has been sent successfully.');
    }
}
