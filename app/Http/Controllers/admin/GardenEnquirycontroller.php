...
class GardenEnquirycontroller extends Controller
{
    public function index()
    {   
        $vendor_id = auth::user()->id;
      
        $data['all_data'] = DB::table('packages_enquiry')->where('count', '<', 5)->whereIn('subservice_id',[77,78])->orderBy('id', 'desc')->get();

        // echo"<pre>";print_r($data['all_data']);echo"</pre>";exit;       
        //echo"<pre>";print_r($data);echo"</pre>";exit;       
       return view('admin.vendor_garden_inquiry',$data);
    }

    function garden_assign_vendor(){
        
        $inquiry_id = $_POST['inquiry_id'];
		
		$garden_enquiry_data = DB::table('garden_enquiry')
                                ->where('id', $inquiry_id)
                                ->first();
                            
        $currentDate = now();
			// echo "<pre>";print_r($garden_enquiry_data);echo"</pre>";exit;
        $html = '<label>Select a Vendor</label>';
        $html .= '<select id="garden_vendor_id" name="garden_vendor_id[]" class="form-control vendor-mul-drop select2" multiple="multiple">';
                
                $html .= "<option value=''>Select Vendor</option>";

                $vendor_data =  DB::table('users')
                                ->whereRaw("FIND_IN_SET(?, serviceList)", [$garden_enquiry_data->service])
                                ->where('vendor', 1)
                                ->where('is_active', 0)
                ...
                foreach($vendor_data as $vendor_data_new){
                    ...
                    if(in_array($vendor_data_new->id,explode(",",$garden_enquiry_data->vendor_id))){
                        $selected = "selected";
                    } else {
                        $selected = "";
                    }
    ...
    function garden_email_vendor(){
        ...
        $html .= '<select id="garden_vendor_id" name="garden_vendor_id[]" class="form-control vendor-mul-drop select2" multiple="multiple">';
                ...
                foreach($vendor_data as $vendor_data_new){
                    ...
                    if(in_array($vendor_data_new->id,explode(",",$garden_enquiry_data->vendor_id_for_email))){
                        $selected = "selected";
                    } else {
                        $selected = "";
                    }
    ...
    function garden_vendor_form(){

        // 
        $vendorId = implode(",",$_POST['garden_vendor_id']);
        ...
        $inquiry_id = $_POST['inquiry_id'];
		$garden_data = DB::table('garden_enquiry')
        ...
        $vendor_detail = DB::table('users')->where('vendor', 1)->whereIn('id',$arrVendor)->get();
        ...
        $service_id = $garden_data->service;
        $subservice_id = $garden_data->subservice;
        ...
        // echo "<pre>";print_r($subscriptionCharge);echo "</pre>";exit;
			...
			{   
				...
				$request_date = $garden_data->service_date;
				...
				$vendor_html .='<!doctype html>
							...
							<div class="email_wrapper" style="width:100%;margin-top: 18px;font-size: 16px;">
									  ...
									  $vendor_html .='<p><strong>Enquiry Details:</strong><br>';
										$vendor_html .='<ul>
															<li><strong>Service: </strong> '.\Helper::servicename($garden_data->service).'</li>
															<li><strong>Garden Service: </strong> '.$garden_data->service_type.'</li>
															<li><strong>Customer Name: </strong> '.$garden_data->user_name.'</li>
															 <li><strong>Customer Email: </strong> '.$garden_data->user_email.'</li>
															<li><strong>Customer Contact: </strong> '.$garden_data->user_mobile.'</li>
					...
					$subject = "New ".\Helper::servicename($garden_data->service)." Enquiry Assigned on VendorsCity | Enquiry No. ".$garden_data->inquiry_id." ";
					$to = $vendordata->email;
                    ...
                    DB::table('garden_enquiry')
                        ->where('id', $inquiry_id)
                        ->update(['vendor_id' => $vendorId]);
                    
					$walletAmountDeduct = $this->garden_wallet_amount_deduct($vendor_id,$service_id,$subservice_id,$inquiry_id);
       ...
       //echo "<pre>";print_r($emptyWalletVendor);echo"</pre>";exit;
        ...
        return redirect()->route('garden-enquiry')->with('success',$successMessage)->with('error',$errorMessage);
    }

    function garden_wallet_amount_deduct($vendor_id,$service_id,$subservice_id,$inquiry_id)
    {  
        $vendor_data = DB::table('users')->where('id',$vendor_id)->first();
        $garden_data = DB::table('garden_enquiry')->where('id', $inquiry_id)->first();
       ...
       if($vendor_package_no_of_inquiry > $vendor_accept_inquiry_data->total_no_of_inquiry){
				...
				{					
                    ...
                    if($subscription_subservice_attribute->charge > 0){
                        ...
                        if($subscription_subservice_attribute->charge > $vendors_data->wallet_amount || $vendors_data->wallet_amount == 0 )
                        {
                            ...
                            $data_inquiry_garden['packages_inquiry_id'] = $inquiry_id;
                            ...
                            $data_inquiry_garden['service_id'] = $service_id;
                            $data_inquiry_garden['subservice_id'] = $subservice_id;
                            $data_inquiry_garden['type_of_leads'] = null;
                            $data_inquiry_garden['size_of_house_id'] = null;
                            $data_inquiry_garden['price_of_lead'] = $subscription_subservice_attribute->charge;
        ...
        if($redirect_type == 'success'){
                ...
                <div class="wrapper" style="width: 100%;max-width:500px;margin:auto;
                    ...
                    <table id="table_new" style="width: 100%;border: 1px solid black;
                        ...
                        <tr>
                            ...
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;border-collapse: collapse;"> '.$garden_data->user_name.'</td>
                        </tr>
                        <tr>
                            <td class="custome_td" style="background-color: #0040E6;
                            ...
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;border-collapse: collapse;"> '.$garden_data->user_email.'</td>
                        </tr>
                        <tr>
                            <td class="custome_td" style="background-color: #0040E6;
                        ...
                        border-collapse: collapse;"> '.$garden_data->user_mobile.'</td>
                        </tr>
						<tr>
                            <td class="custome_td" style="background-color: #0040E6;
                            color: #fff;padding: 10px 60px 10px 12px;border-bottom-color: #fff !important;border: 1px solid black;
                        border-collapse: collapse;">Date:</td>
                            <td class="custome_td_new" style=" padding: 10px 12px;border: 1px solid black;
                        border-collapse: collapse;"> '.$garden_data->service_date.'</td>
                        </tr>';
			...
			$subject = "VendorsCity Garden Lead for ".$garden_data->service_type."! Here Are the Details";
        ...
        $customer_html_new .='<!doctype html>
                ...
                <div class="email_wrapper" style="width:100%;margin-top: 18px;font-size: 16px;">
                   <p>  Dear '.$garden_data->user_name.',</p>
                   <p>We hope this email finds you well.</p>';
                    ...
                    $text2 = "The vendor will contact you soon to confirm the survey details.  If you do not hear from the vendor within the next 24 hours, please contact them directly at ".$garden_data->user_mobile.".";
                    ...
                    $subject_type = "Survey Request for Your " . \Helper::servicename($garden_data->service) ." Request on VendorsCity!";
                  $customer_html_new .='<p>'.$text1 .'<br><br>';
                   ...
                   $customer_html_new .='<span style="
                    margin-bottom: 15px;
                    display: inline-block;
                "><strong>Service Requested: </strong>' .  \Helper::servicename($garden_data->service) .'</span><br>';
               ...
               <div class="email_footer" style="width:100%;margin-top: 20px;">
                            ...
                            float: left;">
                                ...
                                <a href="'.url("/contact").'"  style="width: 100%;color: #555;display: inline-block;">Contact Us</a>
                                    ...
                                    <p style="margin:0;">This message was mailed to '.$garden_data->user_email.' as part of you account registered with us on VendorsCity</p>
                                </div>
                            </div>
                     </div>
                 </div>
    ...
    function garden_wallet_vendor(){
        echo "<pre>";print_r($_POST);echo "</pre>";exit;
        $vendor_id = implode(",",$_POST['garden_vendor_id']);
...
<div class="email_wrapper" style="width:100%;margin-top: 18px;font-size: 16px;">
        ...
        return redirect()->route('garden-enquiry')->with('success','The E-mail to the vendor has been sent successfully.');
    }
}
