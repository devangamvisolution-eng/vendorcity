...
class Ordercontroller extends Controller
{
    ...
    public function index($order_id = '', $status = '')
    {
        ...
        $query = DB::table('ci_orders')->where('ci_orders.is_delete', '0')->where(function ($q) {
            $q->whereNull('ci_orders.is_survey_order')->orWhere('ci_orders.is_survey_order', 0);
        })->where(function ($q) {
            $q->whereNull('ci_orders.is_manpower_order')->orWhere('ci_orders.is_manpower_order', 0);
        })->where(function ($q) {
            $q->whereNull('ci_orders.is_manpower_order')->orWhere('ci_orders.is_manpower_order', 0);
        })
        ...
        $query = DB::table('ci_orders')->where('ci_orders.is_delete', '0')->where(function ($q) {
            ...
            ->select(
                ...
                $subQuery->select(DB::raw(1))
                    ->from('ci_order_item')
                    ->whereColumn('ci_order_item.order_id', 'ci_orders.order_id')
                    ->where('ci_order_item.service_id', 38);
            });
        //$query = $query->where('order_from','!=',2); 
        if (!empty($order_id)) {
            $query->where('ci_orders.order_id', $order_id);
    ...
    public function deductVisitCharge(Request $request)
    {
        ...
        $visit_data = DB::table('ci_order_visits')
            ->where('order_id', $order_id)
            ->where('visit_date', $visit_date)
            ->first();

        $base_charge = $order->order_total ?? 0;
        $extra_charge = $visit_data->extra_charge ?? 0;
        $total_deducted = $base_charge + $extra_charge;

        $existing = \App\Helpers\Helper::getVisitTransaction($order_id, $visit_date);
        if ($existing) {
            return response()->json(['status' => 0, 'message' => 'This visit has already been deducted.']);
        }

        if ($order->paymentmode == 2) {
            $stripe_customer_id = \App\Helpers\Helper::getStripeCustomerId($order->user_id);
            if (!$stripe_customer_id) {
                return response()->json(['status' => 0, 'message' => 'No saved card found for this customer. Cannot auto-deduct.']);
            }
            ...
            try {
                ...
                if (count($payment_methods->data) == 0) {
                    return response()->json(['status' => 0, 'message' => 'Customer has no saved cards in Stripe.']);
                }

                $payment_method_id = $payment_methods->data[0]->id;

                $paymentIntent = $stripe->paymentIntents->create([
                    'amount' => (int) ($total_deducted * 100),
                    'currency' => 'aed',
                    'customer' => $stripe_customer_id,
                ...
                return response()->json(['status' => 0, 'message' => 'Card declined: ' . $e->getError()->message]);
            } catch (\Exception $e) {
                return response()->json(['status' => 0, 'message' => 'Stripe Error: ' . $e->getMessage()]);
            }
        } else {
            $txn_id = 'TXN-ORD' . $order_id . '-VISIT' . str_replace('-', '', $visit_date) . '-' . time();
        }

        DB::table('ci_visit_transactions')->insert([
            'order_id' => $order_id,
    ...
    {
        // Auto-deduct first visit for Stripe orders
        $order = DB::table('ci_orders')->where('order_id', $order_id)->first();
        if ($order && $order->paymentmode == 2) {
            $first_visit = DB::table('ci_order_visits')
                ->where('order_id', $order_id)
                ->orderBy('visit_date', 'asc')
                ->first();

            if ($first_visit) {
                $existing = \App\Helpers\Helper::getVisitTransaction($order_id, $first_visit->visit_date);
                if (!$existing) {
                    $base_charge = $order->order_total ?? 0;
                    $extra_charge = $first_visit->extra_charge ?? 0;
                    $total_deducted = $base_charge + $extra_charge;

                    $txn_id = 'TXN-ORD' . $order_id . '-VISIT' . str_replace('-', '', $first_visit->visit_date) . '-AUTO';

                    DB::table('ci_visit_transactions')->insert([
                        'order_id' => $order_id,
                        'visit_date' => $first_visit->visit_date,
                        'transaction_id' => $txn_id,
                        'amount_deducted' => $total_deducted,
                        'created_at' => now(),
                        'updated_at' => now()
        ...
        foreach ($orderList as $order) {
            $item_id_to_check = !empty($order->order_number) ? $order->order_number : $order->order_id;
            $itemList = DB::table('ci_order_item')
                ->where('order_id', $item_id_to_check)
                ->get();
            if ($order->order_from != 2) {
                ...
                foreach ($itemList as $item) {
                    ...
                    $total += (float) $product_item_price * (float) $item->package_quantity;
                }
                $order->items = $itemList;
...
<div class="email_wrapper" style="width:100%;margin-top: 18px;font-size: 16px;">
    ...
    public function admin_promo_check(Request $request)
    {
        $coupan = $request->promo_code;
        $subtotal = $request->sub_total;
        $subservice_id = $request->sub_service;
        $userid = $request->customer_id ?? 0;

        $service_id = DB::table('subservices')->where('id', $subservice_id)->pluck('serviceid')->first();
        ...
        if ($select_coupan != "") {
            if ($already_applied_promo == "") {
                ...
                if ($select_coupan->no_of_coupons > $numberofcoupon) {
                        ...
                        if ($select_coupan->service_id == "" || $select_coupan->service_id == 0 || ($servicecheck == true && $subservicecheck == true)) {
                            if ($minimum >= $select_coupan->minimum_order) {
                                if ($minimum >= $coupon_discounted) {
                                    return response()->json([
                                        'status' => 'success',
                                        'discount' => $coupon_discounted,
                                        'coupan_name' => $select_coupan->coupan_name,
                                        'coupan_code' => $select_coupan->coupan_code
                                    ]);
                                } else {
                                    return response()->json(['status' => 'error', 'message' => 'grater']);
                                }
                            } else {
                                return response()->json(['status' => 'error', 'message' => $select_coupan->minimum_order]);
                            }
                        } else {
                    ...
                    return response()->json(['status' => 'error', 'message' => 'invalid_user_count']);
                }
            } else {
                return response()->json(['status' => 'error', 'message' => 'Already Used']);
            }
        } else {
            return response()->json(['status' => 'error', 'message' => 'invalid']);
        }
    ...
    public function cleaning_package_order_edit($order_id)
    {
        ...
        // Only override sub_total if it's a package order and the calculated total is greater than 0
        if (count($categoryIds) > 0) {
            $total = 0;
            $orderItemPackages = DB::table('ci_order_item_packages')->where('order_id', $order->order_id)->get();
            foreach ($orderItemPackages as $packageItem) {
                $total += $packageItem->package_item_price * $packageItem->package_quantity;
            }
            if ($total > 0) {
                $order->sub_total = $total;
            }
    ...
    public function car_inspection_order_store(request $request)
    {
        ...
        $coupan_to_wallet = '0';
        $coupon_discounted = $request->promo_discount ?? 0;
        if (!empty($request->promo_name)) {
            $coupon_data = DB::table('coupans')->where('coupan_code', $request->promo_name)->first();
            if ($coupon_data && $coupon_data->coupan_apply_wallet == 0) {
                $coupan_to_wallet = '1';
                $wallet_content = [
                    'userid' => $userid,
                    'refer_id' => $userid,
        ...
        $content = array(
            ...
            'coupon_code' => $request->promo_name ?? '',
    ...
    public function car_services_at_home_service_admin_order()
    {
        ...
        return view('admin.package-orders.carserviceathome.add_order', $data);
    }
        ...
        if ($order_status_value !== "") {
            ...
            if ($order_status_value == "CL") {
                ...
                if ($orderdata && $orderdata->user_id) {
                    $user = DB::table('frontloginregisters')->where('id', $orderdata->user_id)->first();
                    $payment_mode = ($orderdata->paymentmode == 1) ? "COD" : "Online";

                    if ($user && $user->email) {
                        $subject = "Service Booking Cancelled " . $orderdata->format_order_id;
                        $to = $user->email;
                        $bccRecipients = ['hello@vendorscity.com'];

                        try {
                            Mail::send('emails.customer_booking_cancelled', [
                                'orderdata' => $orderdata,
                            ...
                            Log::error('Mail sending failed in order_status_change for cancellation: ' . $e->getMessage());
                        }
                    }
                }
            }

            if ($order_status_value == "BC") {
                $orderdata = DB::table('ci_orders')->where('order_id', $order_id)->first();
                $order_item_data = DB::table('ci_order_item')->where('order_id', $order_id)->get();
                if ($orderdata && $orderdata->user_id) {
                    $user = DB::table('frontloginregisters')->where('id', $orderdata->user_id)->first();
                    ...
                    if ($user && $user->email) {
                        ...
                        try {
                            Mail::send('emails.customer_booking_confirmed', [
                                'orderdata' => $orderdata,
                                'order_item_data' => $order_item_data,
                                'user' => $user,
                                'payment_mode' => $payment_mode
                            ...
                            Log::error('Mail sending failed in order_status_change: ' . $e->getMessage());
                        }
                    }
                    \App\Helpers\Helper::service_confirmation_whatsapp_customer($orderdata->user_id, $order_id);
                }

                // WhatsApp API integration for service confirmation

            }

            if ($order_status_value == "OTW") {
                $orderdata = DB::table('ci_orders')->where('order_id', $order_id)->first();
                if ($orderdata && $orderdata->user_id) {
                    \App\Helpers\Helper::on_the_way_whatsapp_customer($orderdata->user_id, $order_id);
                }
            }

            if ($order_status_value == "CO") {
                $orderdata = DB::table('ci_orders')->where('order_id', $order_id)->first();
                $order_item_data = DB::table('ci_order_item')->where('order_id', $order_id)->get();
                if ($orderdata && $orderdata->user_id) {
                    $user = DB::table('frontloginregisters')->where('id', $orderdata->user_id)->first();
                    ...
                    if ($user && $user->email) {
                        ...
                        $subject = "Your booking summary for your " . $subserviceName . " with VendorsCity";
                        $to = $user->email;
                        $bccRecipients = ['hello@vendorscity.com'];

                        try {
                            Mail::send('emails.customer_booking_completed', [
                                'orderdata' => $orderdata,
                                'order_item_data' => $order_item_data,
                                'user' => $user,
                                'payment_mode' => $payment_mode
                            ...
                            \Log::error('Mail sending failed for order completed: ' . $e->getMessage());
                        }
                    }

                    // Send WhatsApp feedback message
                    \App\Helpers\Helper::post_service_feedback_whatsapp_customer($orderdata->user_id, $order_id);
                }
            }
    ...
    function cleaning_order_store(Request $request)
    {
        ...
        if ($request->subservice_id == '28') {
            ...
            if (!empty($request->promo_name)) {
                $coupon_data = DB::table('coupans')->where('coupan_code', $request->promo_name)->first();
                if ($coupon_data) {
                    if ($coupon_data->coupan_apply_wallet == 0) {
                        $coupan_to_wallet = '1';
            ...
            $data = [
                ...
                'cdate' => date('Y-m-d'),
                'plate_source' => $request->plate_source,
                'plate_code' => $request->plate_code,
                'plate_number' => $request->plate_number,
                'describe_your_car' => $request->describe_your_car,
            ];
            ...
            $content = array(
                ...
                'order_total' => $request->order_total,
                'coupon_code' => $request->promo_name ?? '',
                'coupondiscount' => $request->promo_discount ?? 0,
                'vatcharge' => $request->vat_charge,
                'front_wallet_amount' => '0',
                'order_currency' => 'AED',
                'order_status' => 'BC',
                ...
                'sequence_no' => $nextSequence,
                'format_order_id' => $formatOrderId,
                'custom_subtotal' => $request->custom_subtotal,
            );
            $arrOrderId = DB::table('ci_orders')->insertGetId($content);
            ...
            $formatted_date = date('Y-m-d', strtotime($bookingdate));
            if ($request->subservice_id == 101) {
                if (!empty($request->package_duration_months)) {
                    $end_date = date('Y-m-d', strtotime($formatted_date . ' + ' . $request->package_duration_months . ' months'));
                } else {
                    $end_date = $formatted_date;
                }
            }
            ...
            $data = array(
                ...
                'subservice_id' => $request->subservice_id,
                'how_many_cleaners_do_you_need' => $request->how_many_cleaner,
                'how_many_hours_should_they_stay' => ($request->subservice_id == 101) ? $request->hours : $request->hour_value,
                'how_often_do_you_need_cleaning' => ($request->subservice_id == 101) ? $request->how_often_do_you_need_cleaning : $request->how_often_you_need,
                'do_you_need_cleaning_material' => ($request->subservice_id == 101) ? $request->materials : $request->need_cleaning_material,
                'any_special_instruction' => $request->special_instruction,
                'address_type' => $request->address_type,
                'city' => $request->city,
                'area' => $request->area,
                ...
                'month' => $month,
                'time_slot' => $request->time_slot,
                'which_day_of_the_week_do_you_want_the_service' => implode(',', array_filter($request->which_day_you_want ?? [])),
                'cdate' => date('Y-m-d'),
                'plate_source' => $request->plate_source,
                'plate_code' => $request->plate_code,
                'plate_number' => $request->plate_number,
        ...
        //     $this->send_success_mail_api($arrOrderId, $user_id);
        //     // $this->send_vendor_lead_mail_api($arrOrderId, $user_id);
        // }
            ...
            if ($orderdata && $orderdata->user_id) {
                ...
                if ($user && $user->email) {
                    ...
                    try {
                        ...
                        \Log::error('Mail sending failed in cleaning_order_store: ' . $e->getMessage());
                    }
                }
            }
        ...
        if (!empty($request->promo_name)) {
            ...
            if ($coupon_data) {
                if ($coupon_data->coupan_apply_wallet == 0) {
                    ...
                    // For editing, we might not want to re-credit if already credited, but for now we follow the exact same logic.
                    // Ideally we should check if wallet entry already exists for this order.
                    $existing_wallet = DB::table('front_user_wallet')->where('order_id', $order_id)->where('added_from', 0)->first();
                    if (!$existing_wallet) {
                        $wallet_content = [
                            'userid' => $user_id,
                            'refer_id' => $user_id,
            ...
            if ($orderdata && $orderdata->user_id) {
                ...
                if ($user && $user->email) {
                    ...
                    try {
                        ...
                        \Log::error('Mail sending failed in salon_order_store: ' . $e->getMessage());
                    }
                }
            }
            ...
            if ($orderdata && $orderdata->user_id) {
                ...
                if ($user && $user->email) {
                    ...
                    try {
                        ...
                        \Log::error('Mail sending failed in pest_order_store: ' . $e->getMessage());
                    }
                }
            }
            ...
            if ($orderdata && $orderdata->user_id) {
                ...
                if ($user && $user->email) {
                    ...
                    try {
                        ...
                        \Log::error('Mail sending failed in handy_order_store: ' . $e->getMessage());
                    }
                }
            }
    ...
    function car_services_at_home_service_order_store(Request $request)
    {
        // echo"<pre>";print_r($request->all());exit;
        $intOrderNumber = DB::table('ci_orders')
            ->select(DB::raw('MAX(order_id) as lastOrderNumber'))
            ...
            if ($orderdata && $orderdata->user_id) {
                ...
                if ($user && $user->email) {
                    ...
                    try {
                        ...
                        \Log::error('Mail sending failed in car_service_order_store: ' . $e->getMessage());
                    }
                }
            }
        ...
        return redirect()->route('car-services-at-home-service-order')->with('success', 'Order has been added successfully');
    }
            ...
            if ($orderdata && $orderdata->user_id) {
                ...
                if ($user && $user->email) {
                    ...
                    try {
                        ...
                        \Log::error('Mail sending failed in health_order_store: ' . $e->getMessage());
                    }
                }
            }
    ...
    function automobile_order_store(Request $request)
    {
        ...
        if ($request->subservice_id != '') {
            ...
            if (!empty($coupon_code)) {
                $coupon_data = DB::table('coupans')->where('coupan_code', $coupon_code)->first();
                if ($coupon_data && $coupon_data->coupan_apply_wallet == 0) {
                    $coupan_to_wallet = '1';
                    $wallet_content = [
                        'userid' => $user_id,
                        ...
                        'order_total' => $request->order_total,
                        'system_percentage' => '',
                        'wallet_amount' => $coupon_discounted ?? 0,
                        'added_from' => 0,
                        'order_id' => $intOrderNumber_new,
                        'added_date' => date('Y-m-d'),
                    ];
            ...
            if ($orderdata && $orderdata->user_id) {
                ...
                if ($user && $user->email) {
                    ...
                    try {
                        ...
                        \Log::error('Mail sending failed in automobile_order_store: ' . $e->getMessage());
                    }
                }
            }
    ...
    function painting_service_order_store(Request $request)
    {
        ...
        if ($request->subservice_id != '') {
            ...
            //     // $this->success_whatsapp_message($userid, $arrOrderId);
            //     $this->send_success_mail_api($arrOrderId, $userid);
            //     // $this->send_vendor_lead_mail_api($arrOrderId, $userid);
            // }
            if ($request->send_notification == 'yes') {
                ...
                if ($orderdata && $orderdata->user_id) {
                    ...
                    if ($user && $user->email) {
                        ...
                        try {
                            ...
                            \Log::error('Mail sending failed in painting_order_store: ' . $e->getMessage());
                        }
                    }
                }
...
<p>Your renewal details are as follows:</p>';
    ...
    public function send_success_mail_api($order_id, $user_id)
    {
        $user = DB::table('frontloginregisters')->where('id', $user_id)->first();
        $orderdata = DB::table('ci_orders')->where('order_id', $order_id)->first();
        $order_item_data = DB::table('ci_order_item')->where('order_id', $order_id)->get();
    ...
    public function send_vendor_lead_mail_api($order_id, $user_id)
    {
        $user = DB::table('frontloginregisters')->where('id', $user_id)->first();
...
<div class="wrapper" style="width: 100%;max-width:500px;margin:auto;
    ...
    function moving_order_store(Request $request)
    {
        ...
        try {
            ...
            $coupan_to_wallet = '0';
            $coupon_discounted = $request->coupon_discounted ?? 0;
            if (!empty($request->coupon_code)) {
                $coupon_data = DB::table('coupans')->where('coupan_code', $request->coupon_code)->first();
                if ($coupon_data && $coupon_data->coupan_apply_wallet == 0) {
                    $coupan_to_wallet = '1';
                    $wallet_content = [
                        'userid' => $request->customer_id,
                        'refer_id' => $request->customer_id,
                        'order_currency' => 'AED',
                        'order_total' => $request->order_total,
                        'system_percentage' => '',
                        'wallet_amount' => $coupon_discounted,
            ...
            //     $this->send_success_mail_api($arrOrderId, $request->customer_id);
            //     // $this->send_vendor_lead_mail_api($arrOrderId, $request->customer_id);
            // }
            if ($request->send_notification == 'yes') {
                ...
                if ($orderdata && $orderdata->user_id) {
                    ...
                    if ($user && $user->email) {
                        ...
                        try {
                            ...
                            \Log::error('Mail sending failed in moving_order_store: ' . $e->getMessage());
                        }
                    }
                }
    ...
    public function moving_order_update(Request $request, Ciorder $ci_order)
    {
        ...
        try {
            ...
            $content = [
                ...
                'sub_total' => $request->sub_total,
                'coupon_code' => $request->coupon_code,
                'coupondiscount' => $request->coupon_discounted,
                'order_status' => 'BC',
                'payment_status' => 'Success',
                'coupan_to_wallet' => "0",
                'moving_date' => $request->moving_date,
    ...
    public function car_services_at_home_order_edit(Ciorder $ci_order)
    {
        $data['error'] = '';
        ...
        $itemList = CiorderItem::orderId($ci_order->order_id)->serviceId(38)->get();
        ...
        $data['ev_charger_type'] = DB::table('ev_charger_type')->orderBy('set_order', 'asc')->get();
        $data['ev_charger_location_type'] = DB::table('ev_charger_location_type')->orderBy('set_order', 'asc')->get();
        return view('admin.package-orders.carserviceathome.edit_order', $data);
    }

    public function car_services_at_home_order_update(Request $request, Ciorder $ci_order)
    {
        DB::beginTransaction();
        try {
            $user_id = $request->customer_id;
            ...
            return redirect()->route('car-services-at-home-service-order')->with('success', 'Order has been updated successfully');
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Car Services at Home Order update failed: " . $e->getMessage());
            return redirect()->back()->with('error', 'Update failed: ' . $e->getMessage())->withInput();
        }
    }
                    ...
                    if ($orderdata && $orderdata->user_id) {
                        ...
                        if ($user && $user->email) {
                            ...
                            try {
                                ...
                                \Log::error('Mail sending failed in storage_order_store: ' . $e->getMessage());
                            }
                        }
                    }
 ...
 <style>
    ...
    public function adjust_visit_hours(Request $request)
    {
        $visit_date = $request->input('visit_date');
        $order_id = $request->input('order_id');
        $extra_hours = $request->input('extra_hours');
        $manual_extra_charge = $request->input('manual_extra_charge');

        $adjustment_type = $request->input('adjustment_type', 'add');

        if ($visit_date && $order_id && isset($extra_hours)) {
            $order = DB::table('ci_orders')->where('order_id', $order_id)->first();

            $hourly_rate = 0;
            if ($order && isset($order->cleaner_per_hour_charge) && $order->cleaner_per_hour_charge > 0) {
                $hourly_rate = $order->cleaner_per_hour_charge;
            }

            if (isset($manual_extra_charge) && $manual_extra_charge !== '') {
                $extra_charge = floatval($manual_extra_charge);
            } else {
                $extra_charge = floatval($extra_hours) * floatval($hourly_rate);
            }

            if ($adjustment_type === 'deduct') {
                $extra_hours = -abs(floatval($extra_hours));
                $extra_charge = -abs(floatval($extra_charge));
            } else {
                $extra_hours = abs(floatval($extra_hours));
                $extra_charge = abs(floatval($extra_charge));
            }
        ...
        return response()->json(['status' => 0, 'message' => 'Visit date, Order ID or Extra Hours missing']);
    }
}
