        ...
        if ($request->subservice_id == 23 || $request->subservice_id == 26 || $request->subservice_id == 53 || $request->subservice_id == 94 || $request->subservice_id == 100 || $request->subservice_id == 98) {
            ...
            if ($data['form_type'] == 'Local Move') {
                ...
                foreach ($formFieldIds as $index => $fieldId) {
                    if ($fieldId == 17) {
                        $cityOptionId = $formFieldVals[$index] ?? null;
                        break;
                    }
                }
                ...
                foreach ($formFieldIds as $index => $fieldId) {
                    if ($fieldId == 57) {
                        $countryOptionId = $formFieldVals[$index] ?? null;
                        break;
                    }
                }
            ...
            foreach ($request->form_field_id as $key => $value) {
                if (!empty($value) && isset($request->formfield_value[$key])) {
                    $data1 = [
                        ...
                        'form_field_id' => $value,
                        'formfield_value' => $request->formfield_value[$key]
                    ];
        ...
        if ($request->form_field_radio_id_one != '' && count($request->form_field_radio_id_one) > 0) {
            foreach ($request->form_field_radio_id_one as $key1 => $values1) {
                ...
                if ($request->form_field_radio_id_one[$key1] != '') {
                    $data2 = [
                        'package_inquiry_id' => $package_inquiry,
                        'form_field_id' => $request->form_field_radio_id_one[$key1],
                        'formfield_value' => $request['formfield_radio_' . $radioVal] ?? null
                    ];
        ...
        if ($request->form_field_checkbox_id_one != '' && count($request->form_field_checkbox_id_one) > 0) {
            foreach ($request->form_field_checkbox_id_one as $key1 => $values1) {
                ...
                if ($request->form_field_checkbox_id_one[$key1] != '') {
                    $data3 = [
                        'package_inquiry_id' => $package_inquiry,
                        'form_field_id' => $request->form_field_checkbox_id_one[$key1],
                        'formfield_value' => $request['formfield_checkbox_' . $ckeckboxVal] ?? null
                    ];
        ...
        if ($request->form_field_mul_dropdown_id != '' && count($request->form_field_mul_dropdown_id) > 0) {
            foreach ($request->form_field_mul_dropdown_id as $key1 => $values1) {
                ...
                if ($request->form_field_mul_dropdown_id[$key1] != '') {
                    $data4 = [
                        'package_inquiry_id' => $package_inquiry,
                        'form_field_id' => $request->form_field_mul_dropdown_id[$key1],
                        'formfield_value' => $request['formfield_mul_dropdown_' . $Multiple_drop_down_Val] ?? null
                    ];
