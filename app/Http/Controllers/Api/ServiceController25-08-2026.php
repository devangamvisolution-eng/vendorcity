...
class ServiceController extends Controller
{
    ...
    public function get_enquiry_form_fields(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'service_id' => 'required',
        ...
        // If subservice is '31' (Storage), service might be null in old logic, but let's just fetch it anyway or replicate the logic
        if ($subservice_id != '31') {
        ...
        // Separate Local Fields (form_fields) and International Fields (form_fields_two)
        $local_tags = [];
        $international_tags = [];
        
        if ($form_field_data_ser) {
            $ser_local = $form_field_data_ser->form_fields ? explode(',', $form_field_data_ser->form_fields) : [];
            $ser_intl = isset($form_field_data_ser->form_fields_two) && $form_field_data_ser->form_fields_two ? explode(',', $form_field_data_ser->form_fields_two) : [];
            $local_tags = array_merge($local_tags, $ser_local);
            $international_tags = array_merge($international_tags, $ser_intl);
        }

        if ($form_field_data_sub) {
            $sub_local = $form_field_data_sub->form_fields ? explode(',', $form_field_data_sub->form_fields) : [];
            $sub_intl = isset($form_field_data_sub->form_fields_two) && $form_field_data_sub->form_fields_two ? explode(',', $form_field_data_sub->form_fields_two) : [];
            $local_tags = array_merge($local_tags, $sub_local);
            $international_tags = array_merge($international_tags, $sub_intl);
        }

        $local_tags = array_filter(array_unique($local_tags));
        $international_tags = array_filter(array_unique($international_tags));

        // Helper function to format fields
        $formatFields = function($tags) {
            $fields = [];
            if (!empty($tags)) {
                $result = DB::table('form_fileds')->whereIn('id', $tags)->orderBy('set_order')->get();

                foreach ($result as $field) {
                    $options = [];
                    if (in_array($field->type, ['2', '3', '4', '6'])) {
                        $rawOptions = DB::table('form_attributes')
                        ...
                        foreach ($rawOptions as $opt) {
                            $dependent_fields_raw = DB::table('more_form_attributes')
                                ->where('form_id', $field->id)
                                ->where('attr_id', $opt->id)
                                ->get();

                            $dependent_fields = [];
                            if (count($dependent_fields_raw) > 0) {
                                $sub_options = [];
                                foreach ($dependent_fields_raw as $dep) {
                                    $sub_options[] = [
                                        'id' => $dep->id,
                                        'more_form_option' => $dep->more_form_option
                                    ];
                                }
                                
                                $is_multiple = ($field->id == 35 && $opt->id == 111) ? true : false;
                                
                                $dependent_fields[] = [
                                    'label_name' => 'What is the size of your home?',
                                    'type' => '3',
                            ...
                            $options[] = [
                                'id' => $opt->id,
                                'form_option' => $opt->form_option,
                                'dependent_fields' => empty($dependent_fields) ? null : $dependent_fields
                            ];
                        }
                    }
        ...
        $local_fields = $formatFields($local_tags);
        $international_fields = $formatFields($international_tags);

        return response()->json([
            'status' => true,
            'message' => 'Form fields fetched successfully.',
            'data' => [
                'service_name' => $form_field_data_ser ? $form_field_data_ser->servicename : '',
                'subservice_name' => $form_field_data_sub ? $form_field_data_sub->subservicename : '',
                'local_fields' => $local_fields,
                'international_fields' => $international_fields
            ]
        ]);
    }
}
