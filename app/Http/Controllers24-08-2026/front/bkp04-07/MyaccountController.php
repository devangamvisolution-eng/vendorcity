...
class MyaccountController extends Controller
{
    ...
    $userid = $userdata['userid']; */
        ...
        foreach ($baseQuery as $order) {
            ...
            if (!$item || !isset($item->bookingyear)) continue;

            $total = 0;
            foreach ($items as $it) {
                $it = (object)$it;
                $price = $it->product_discount_amount ?: $it->package_item_price;
                $total += (float)$price * (int)$it->package_quantity;
            }
            $order->sub_total = $total;

            $howOften = $item->how_often_do_you_need_cleaning;
            $startDateStr = $item->bookingdate . ' ' . $item->month . ' ' . $item->bookingyear;
            ...
            $endDate = (!empty($item->end_date)) ? Carbon::parse($item->end_date) : $startDate->copy();
            ...
            if ($howOften == 'Weekly') {
            ...
            } elseif ($howOften == 'Multiple times a week' || $howOften == 'multiple times a week' || $howOften == 'Multiple Times A Week') {
                ...
                // Once or deep cleaning or others
            ...
            foreach ($visitDates as $vDate) {
                if ($vDate < $today) {
                    $latestPastVisit = $vDate; // visitDates is ascending, so latest stays latest
                } else if (!$nextUpcomingVisit) {
                    $nextUpcomingVisit = $vDate;
                }
            }

            if ($latestPastVisit || ($order->order_status == 'CO' && $nextUpcomingVisit)) {
                $vClone = clone $order;
                // If it's marked completed but has a next upcoming visit, we use that as the "final" visit for display in Past
                // or just use the latest past visit if available.
                $FinalVisitDate = $latestPastVisit ?: $nextUpcomingVisit;
                $vClone->visit_date = $FinalVisitDate;

                // Fetch tips for this specific visit
                $vClone->tips = DB::table('ci_tips')
                    ->where('order_id', $order->order_id)
                    ->where('visit_date', $FinalVisitDate)
                    ->where('payment_status', 'paid')
                    ->get();
                $vClone->total_tipped = $vClone->tips->sum('tip_amount');

                $pastCollection->push($vClone);
            }

            if ($nextUpcomingVisit && $order->order_status != 'CL' && $order->order_status != 'CO') {
                $vClone = clone $order;
                $vClone->visit_date = $nextUpcomingVisit;
                ...
                $upcomingCollection->push($vClone);
            }
        }

        // Sort collections by order_id DESC as requested
        $pastCollection = $pastCollection->sortByDesc('order_id');
        $upcomingCollection = $upcomingCollection->sortByDesc('order_id');
...
if (empty($userdata)) {
    return redirect()->to('/Sign-in');
        ...
        foreach ($itemList as $item) {
            ...
            $total += (float)$product_item_price * (int)$item->package_quantity;
        ...
        $query->visit_date = $visit_date; // Pass to view

        // Calculate past visits for recurring orders
        $item = (object)$itemList->first();
        if ($item && isset($item->bookingyear)) {
            $howOften = $item->how_often_do_you_need_cleaning;
            $startDateStr = $item->bookingdate . ' ' . $item->month . ' ' . $item->bookingyear;
            ...
            foreach ($visitDates as $vDate) {
                if ($vDate < $today) {
                    // Check if tipped for this visit
                    $tip = DB::table('ci_tips')
                        ->where('order_id', $id)
            ...
            $data['pastVisits'] = $pastVisits;
        }

        // Get tips for this specific visit or general
        $tipsQuery = DB::table('ci_tips')
            ->where('order_id', $query->order_id)
            ->where('payment_status', 'paid');

        if ($visit_date) {
            $tipsQuery->where('visit_date', $visit_date);
        }

        $query->tips = $tipsQuery->get();
        $query->total_tipped = $query->tips->sum('tip_amount');

        // Check if this specific visit has been reviewed
        $checkReview = null;
        if ($visit_date) {
            ...
            // General review check for the order (fallback or for one-time orders)
            $checkReview = DB::table('ci_cleaners_review')
                ->where('order_id', $id)
                ->first();
        }
        ...
        // Return to view
        $query->visit_date = $visit_date; // Assign for UI
        $data['orders'] = $query;
        $data['today'] = $today;
    ...
    public function order_add_tip(Request $request)
    {
        ...
        if ($tip_amount <= 0) {
            return redirect()->back()->with('error', 'Please enter a valid tip amount.');
        }
        ...
        // Create a record in ci_tips
        $tip_id = DB::table('ci_tips')->insertGetId([
            'order_id' => $order_id,
        ...
        $session = StripeSession::create([
            ...
            'line_items' => [[
                'price_data' => [
                    'currency' => 'aed',
                    'product_data' => [
                        'name' => 'Tip for Order #' . ($order->format_order_id ?: $order->order_id),
                    ],
                    'unit_amount' => $tip_amount * 100, // cents
                ],
                'quantity' => 1,
            ]],
            'mode' => 'payment',
            'success_url' => url('/tip-success/' . $tip_id),
            'cancel_url' => url('/tip-cancel/' . $tip_id),
        ]);

        DB::table('ci_tips')->where('id', $tip_id)->update(['stripe_session_id' => $session->id]);

        return redirect($session->url);
    }

    public function tip_payment_success(Request $request, $id)
    {
        ...
        Stripe::setApiKey(config('stripe.stripe_sk'));
        $session = StripeSession::retrieve($tip->stripe_session_id);

        DB::table('ci_tips')->where('id', $id)->update([
            'payment_status' => 'paid',
    ...
    function view_receipts($id, Request $request)
    {
        $visit_date = $request->get('visit_date');

        // Copy watermark / monogram logo assets if not present
        $colorMonogramDest = public_path('site/images/vc-monogram-color.png');
        $whiteMonogramDest = public_path('site/images/vc-monogram-white.png');

        if (!file_exists($colorMonogramDest)) {
            $src = 'C:\\Users\\AE\\.gemini\\antigravity-ide\\brain\\3c0524cf-d485-473c-9719-7810854d3971\\media__1780986124454.png';
            if (file_exists($src)) {
                @copy($src, $colorMonogramDest);
            }
        }
        if (!file_exists($whiteMonogramDest)) {
            $src = 'C:\\Users\\AE\\.gemini\\antigravity-ide\\brain\\3c0524cf-d485-473c-9719-7810854d3971\\media__1780986124432.png';
            if (file_exists($src)) {
                @copy($src, $whiteMonogramDest);
            }
        }
        ...
        if ($request->get('download') == 'pdf') {
            ...
            $html = view('front.view_receipts', $data)->render();
            ...
            $fileName = 'Receipt-' . ($query->order_number ?? $id) . '.pdf';
            return response()->streamDownload(function () use ($mpdf) {
                echo $mpdf->Output('', 'S');
            }, $fileName);
        }

        return view('front.view_receipts', $data);
    }
    ...
    function submitreview(Request $request)
    {
        ...
        $request->validate([
            ...
            'order_id' => 'required|exists:ci_orders,order_id',
        ...
        $itemData = DB::table('ci_order_item')->where('order_id', $orderId)->first();

        if (!empty($itemData)) {

            $cleanresId = explode(',', $itemData->cleaner_id);

            if (isset($cleanresId)) {
                foreach ($cleanresId as $key => $id) {
                    $data['order_id'] = $orderId;
                    $data['visit_date'] = $visitDate;
                    $data['crew_id'] = $id;
                    $data['rating'] = $request->rating;
                    $data['message'] = $request->content;
                    DB::table('ci_cleaners_review')->insert($data);
                }
