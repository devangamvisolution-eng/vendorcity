...
class Packagecontroller extends Controller
{
    ...
    public function enquiry(Request $request, $service_param)
    {
        if (is_numeric($service_param)) {
            $service = \App\Models\Admin\Service::findOrFail($service_param);
        ...
        $service = \App\Models\Admin\Service::where('page_url', $service_param)->firstOrFail();
    ...
    public function booknow(Request $request, $service_param, $subservice_param)
    {
        if (is_numeric($service_param) || is_numeric($subservice_param)) {
            $service = is_numeric($service_param) 
                ? \App\Models\Admin\Service::findOrFail($service_param) 
                : \App\Models\Admin\Service::where('page_url', $service_param)->firstOrFail();
                
            $subservice = \App\Models\Admin\Subservice::where(is_numeric($subservice_param) ? 'id' : 'page_url', $subservice_param)
        ...
        $service = \App\Models\Admin\Service::where('page_url', $service_param)->firstOrFail();
        $subservice = \App\Models\Admin\Subservice::where('page_url', $subservice_param)->where('serviceid', $service->id)->firstOrFail();
