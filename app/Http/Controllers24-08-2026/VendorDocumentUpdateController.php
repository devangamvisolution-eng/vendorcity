...
class VendorDocumentUpdateController extends Controller
{
    ...
    public function show(Request $request, $vendorId)
    {
        ...
        if (count($expiringDocuments) === 0) {
            return redirect('vendor/login')->with('success', 'Your documents are already up to date!');
        }
    ...
    public function update(Request $request, $vendorId)
    {
        ...
        return redirect('vendor/login')->with('success', 'Your documents have been successfully updated! You can now log in.');
    }
}
