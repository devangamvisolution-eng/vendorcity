        ...
        foreach ($expiredItems as $item) {
            ...
            try {
                ...
                if (empty($paymentMethods->data)) {
                    $this->error('No payment methods found for Customer ID: ' . $user->stripe_customer_id);
                    continue;
                }
                ...
                if ($paymentIntent->status == 'succeeded') {
                    ...
                    try {
                        ...
                        // Prefer the actual booking start date fields; only fall back to cdate if they're genuinely missing
                        $hasBookingFields = !empty($item->bookingyear) && !empty($item->month) && !empty($item->bookingdate);

                        $oldStartDate = $hasBookingFields
                            ? Carbon::parse($item->bookingyear . '-' . $item->month . '-' . $item->bookingdate)
                            : Carbon::parse($item->cdate);
