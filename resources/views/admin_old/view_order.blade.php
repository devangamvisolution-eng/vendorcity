...
@php
    $statusDetails = \App\Helpers\Helper::getOrderStatusDetails($order->order_status);
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div>
                            <span class="badge {{ $statusColor }} mb-2" style="font-size: 0.85rem; padding: 0.5em 1em;">
                                {{ $statusText }}
                            </span>
                ...
                <div class="card-body">
                    <div class="row g-4">
                        @foreach ($order->items as $item)
                            ...
                            <div class="col-md-6 col-xl-4">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <span
                                            class="fw-bold">{!! Helper::subservicename(strval($item->subservice_id)) !!}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            @endif
                            @if (
                                    isset($item->which_day_of_the_week_do_you_want_the_service) &&
                            ...
                            @if ($item->subservice_id == '92')
                                ...
                                @if ($item->verifybuy_additional_details != '')
                                    <div class="col-md-6 col-xl-4">
                                        ...
                                        <span class="fw-bold text-dark">{{ $item->verifybuy_additional_details ?: '-' }}</span>
                                    </div>
                                @endif
                                @if ($item->verifybuy_where_is_car_parked != '')
                                    <div class="col-md-6 col-xl-4">
                                        <p class="text-muted mb-0 small">Where is Car Parked?</p>
                                        <span class="fw-bold text-dark">{{ $item->verifybuy_where_is_car_parked ?: '-' }}</span>
                                    </div>
                                @endif
                    ...
                    <div class="vertical-timeline ps-3 mt-2">
                        ...
                        @else
                            @foreach ($normalFlow as $key => $label)
                                ...
                                <div class="timeline-item pb-4 {{ $isLast ? '' : 'border-start' }} ps-4 position-relative">
                                    <span
            ...
            @if($recurring_visits->count() > 0)
                                            ...
                                            <td class="text-end">
                                                @php
                                                    ...
                                                    if ($order->paymentmode == 2) {
                                                        $txn = \App\Helpers\Helper::getVisitTransaction($order->order_id, $visit->visit_date);
                                                    }
                                                ...
                                                @if($txn)
                                                    <div class="mb-1 text-end">
                                                        <span class="badge bg-success" style="font-size: 0.75rem;">Cut:
                                                            {{ number_format($txn->amount_deducted, 2) }} AED</span><br>
                                                    ...
                                                    <div class="dropdown mt-1">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                                                            @if($order->paymentmode == 2 && !$txn && $visit->visit_status != 'cancelled' && $visit->visit_status != 'skipped')
                                                                <li>
                                                                    <button class="dropdown-item text-success deduct-visit-btn"
                                                                        type="button" data-date="{{ $visit->visit_date }}"
                                                                        data-order="{{ $order->order_id }}" data-base="{{ $basePrice }}"
                                                                        data-extra="{{ $visit->extra_charge ?? 0 }}">
                                                                        <i class="fas fa-hand-holding-usd me-2"></i> Deduct
                                                                    </button>
                                                                </li>
                                                                <li>
                                                            ...
                                                            @endif
                                                            <li>
                                                                <button class="dropdown-item text-primary assign-cleaner-btn"
                                                                    type="button" data-date="{{ $visit->visit_date }}"
                                                            ...
                                                            <li>
                                                                <button class="dropdown-item text-warning adjust-hours-btn"
                                                                    type="button" data-date="{{ $visit->visit_date }}"
                                                                    data-order="{{ $order->order_id }}"
                                                            ...
                                                            <li>
                                                                <button class="dropdown-item text-danger cancel-visit-btn"
                                                                    type="button" data-date="{{ $visit->visit_date }}"
                                                                    data-order="{{ $order->order_id }}">
                ...
                <div class="modal fade" id="assignCleanerModal" tabindex="-1" aria-labelledby="assignCleanerModalLabel"
                    aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" step="0.5" class="form-control" id="adjust_extra_hours"
                                        placeholder="e.g. 2">
                                    <small class="text-muted">Enter the number of hours.</small>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Amount (AED)</label>
                                    <input type="number" step="0.01" class="form-control" id="adjust_extra_charge"
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.status == 1) {
                                        ...
                                        $('#saveAdjustHoursBtn').prop('disabled', false).text('Save Adjustment');
                                    }
                                },
                                error: function (err) {
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.status == 1) {
                                        ...
                                        Swal.fire('Error!', response.message, 'error');
                                        $('#saveAssignedCleanerBtn').prop('disabled', false).text('Save Assignment');
                                    }
                                },
                                error: function (err) {
                                    Swal.fire('Error!', 'Something went wrong', 'error');
            ...
            @php
                $past_visits = \App\Helpers\Helper::getUpcomingVisits($order->order_id, 100, 'past')->sortByDesc('visit_date');
            @endphp
                                            ...
                                            <td class="text-end">
                                                @if($order->paymentmode == 2)
                                                    ...
                                                    @elseif($visit->visit_status != 'cancelled' && $visit->visit_status != 'skipped')
                                                        <button class="btn btn-sm btn-outline-success mb-1 deduct-visit-btn w-100"
                                                            type="button" data-date="{{ $visit->visit_date }}"
                                                            data-order="{{ $order->order_id }}" data-base="{{ $basePrice }}"
                                                            data-extra="{{ $visit->extra_charge ?? 0 }}">
        ...
        <div class="col-lg-4">
            ...
            @php
                ...
                if (\Illuminate\Support\Facades\Schema::hasTable('ci_order_visits')) {
                    $orderIdForVisits = !empty($order->format_order_id) ? $order->format_order_id : $order->order_id;
                    $recurring_visits = DB::table('ci_order_visits')->where('order_id', $orderIdForVisits)->orderBy('visit_date', 'asc')->get();
                }
            @endphp
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                ...
                                                @elseif($visit->payment_status == 'pending' && $order->paymentmode == 1)
                                                    <span class="badge bg-warning text-dark"><i class="fas fa-money-bill me-1"></i>
                                                        COD Pending</span>
                                                @else
                                            ...
                                            <td class="text-end px-4 align-middle">
                                                @if($visit->visit_status != 'cancelled' && $visit->visit_status != 'completed')
                                                    ...
                                                    <button class="btn btn-sm btn-outline-danger admin-cancel-visit ms-1"
                                                        data-id="{{ $visit->id }}">
