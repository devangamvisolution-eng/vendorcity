...
<div class='content container-fluid'><div class='page-header'><div class='row align-items-center'><div class='col'><h3 class='page-title'>Add CleaningSubscriptionPricing</h3></div><div class='col-auto'><a class='btn btn-primary btn-premium' href="{{ route('cleaning-subscription-pricing.index') }}">Back</a></div></div></div><div class='row'><div class='col-md-12'><div class='card premium-card'><div class='card-body'><form action="{{ route('cleaning-subscription-pricing.store') }}" method='POST'>@csrf <div class='row'><div class='col-md-6 mb-3'>
    <label>Duration</label>
    <select name='duration_id' class='form-control' required>
        <option value="">Select Duration</option>
        @foreach($durations as $duration)
            <option value="{{ $duration->id }}">{{ $duration->hours }} Hours</option>
        @endforeach
    </select>
</div>
