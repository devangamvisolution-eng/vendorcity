...
<div class='content container-fluid'><div class='page-header'><div class='row align-items-center'><div class='col'><h3 class='page-title'>Cleaning Subscription Pricing Rules</h3><ul class='breadcrumb'><li class='breadcrumb-item'><a href="{{ url('/admin') }}">Dashboard</a></li><li class='breadcrumb-item active'>Cleaning Subscription Pricing Rules</li></ul></div>@if(in_array('80', $edit_perm))<div class='col-auto'><a class='btn btn-primary btn-premium' href="{{ route('cleaning-subscription-pricing.create') }}"><i class='fas fa-plus'></i> Add New</a></div>@endif</div></div><div class='row'><div class='col-sm-12'><div class='card premium-card'><div class='card-body p-0'><div class='table-responsive'><table class='table premium-table'><thead><tr><th>ID</th><th>Duration id</th><th>Frequency id</th><th>Package id</th><th>Price per hour</th><th>Order</th><th>Web</th><th>App</th><th>Action</th></tr></thead><tbody>@foreach($data as $row)<tr><td>{{ $row->id }}</td>
<td>{{ isset($durations[$row->duration_id]) ? $durations[$row->duration_id]->hours . ' Hours' : $row->duration_id }}</td>
<td>{{ isset($frequencies[$row->frequency_id]) ? $frequencies[$row->frequency_id]->label : $row->frequency_id }}</td>
<td>{{ isset($packages[$row->package_id]) ? $packages[$row->package_id]->name : $row->package_id }}</td>
...
<td>{{ $row->set_order }}</td>
    ...
    <div class="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">
            @if(in_array('80', $edit_perm))
            <li>
                <a class="dropdown-item py-2" href="{{ route('cleaning-subscription-pricing.edit', $row->id) }}">
                    <i class="fas fa-edit text-primary me-2"></i> Edit
                </a>
            </li>
            <li>
                <form id="delete-form-{{ $row->id }}" action="{{ route('cleaning-subscription-pricing.destroy', $row->id) }}" method="POST" style="display:inline;" class="delete-form">
                    @csrf
                    @method('DELETE')
