               ...
               <div class="row align-items-center">
                   <div class="col">
                       ...
                       <ul class="breadcrumb">
                           ...
                           <li class="breadcrumb-item active">Description Of Goods</li>
                       </ul>
                   </div>
                    @if (in_array('72', $edit_perm))
                       <div class="col-auto">
                           <a class="btn btn-primary me-1" href="{{ route('erp_dog.create') }}">
           ...
           <div class="row">
               <div class="col-sm-12">
                   ...
                   <div class="card card-table">
                       <div class="card-body">
                           <form id="form" action="{{ route('erp_dog.delete') }}" enctype="multipart/form-data">
                                                   ...
                                                   @if (in_array('72', $edit_perm))
                                                       <td class="text-right">
                                                           <a class="btn btn-primary"
                                                               href="{{ route('erp_dog.edit', $data->id) }}"><i
    ...
    <div class="modal custom-modal fade" id="document_model" role="dialog">
       ...
       <script>
        ...
        function viewdocument(id){
            ...
            $('#document_model_body').html('<p>Loading...</p>');

            $.ajax({
           url: "{{ route('erp_quote.getSurveydocument', ':id') }}".replace(':id', id),   // Laravel route
            type: "GET",
            success: function(response) {
                if (response.success && response.data.length > 0) {
                let html = '<ul class="list-group">';
                response.data.forEach(function(doc) {
                    // If you stored only filename in DB
                    let fileUrl = "{{ asset('public/upload/erp_vendor_surveydocuments') }}/" + doc.document;
                ...
                $('#document_model_body').html('<p>No documents found</p>');
            }
            },
            error: function() {
                $('#document_model_body').html('<p>Error loading data.</p>');
            }
        });
