                    ...
                    <div class="col-sm-6 col-xl-4">
                        <a href="{{ route('automobile.bookinspection', $data->id) }}">
                            <div class="pricing_packages text-center bdrs16">
                                ...
                                <div class="tag_hig">
                                    @if (!empty($data->tag))
                                        <p class="tag_title "><i class="fa-solid fa-star"></i> {{ $data->tag }}</p>
                                    @endif
                                </div>

                                <div class="heading mt10 mb10">

                                    <h1 class="package_title ">{{ $data->name }}</h1>
                                    <h4 class="text1 mt-2"> <img class="dhiram"
                                            src="{{ asset('public/site/images/automobile/dhiram.png') }}"
                                            alt="">{{ $data->price }} <small></small></h4>
                                    <p style="color: #000;">Onwards</p>
                                </div>
                                <div class="details">
                                    @php
                                        $package_attr = App\Models\Admin\VerifyBuyPackageAttr::where(
                                            'verify_buy_package_id',
                                            $data->id,
                                        )
                                            ->orderBy('id', 'DESC')
                                            ->get();
