...
<style>
    ...
    .calendar-day.disabled {
        opacity: 0.5;
        /* Optional: Lowers opacity to enhance the disabled effect */
    }
    ...
    #days-wrapper {
        ...
        /* Smooth scrolling */
        cursor: grab;
        /* Change cursor to grab for desktop users */
    ...
    padding-right: 10px;
        ...
        .modal-dialog {
            max-width: 60%;
            /* Keep the width as needed */
            height: 70%;
            /* Height of the entire modal */
        }
        ...
        .modal-body {
            ...
            /* Adjust for modal header and footer */
            overflow-y: auto;
            -webkit-overflow-scrolling: touch;
    ...
    .surcharge-badge-dayslot span .currency_dhiram {
        margin: 0;
        color: #000;
    }
    ...
    .package-category-class .mobile-furniture {
        padding-left: 0;
    }
    ...
    #sticky-header {
        ...
        max-width: 100%;
        /* Prevent it from growing outside */
        background-color: white;
        box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
        z-index: 8;
        padding: 10px;
    ...
    .radio-slider-wrapper {
        ...
        gap: 0.75rem;
        padding: 10px 0;
        /* Remove horizontal padding, avoid overlap */
    }
...
<section class="our-register">
    <div class="container">
        ...
        <div id="service-step-title" class="main-title">
            ...
            <h2 class="title step-title">Reschedule</h2>
        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-md-9">
                ...
                <form id="reschedule_form" action="{{ route('reschedule-from', $ci_order_item->order_id) }}"
                    method="POST">
                    @csrf
                    <input type="hidden" name="old_order_id" id="old_order_id" value="{{ $ci_order_item->order_id }}">
                    <input type="hidden" name="ci_cleaner_id" id="ci_cleaner_id"
                        value="{{ $ci_order_item->cleaner_id }}">
                    ...
                    <input type="hidden" name="ci_how_many_hours_should_they_stay"
                        id="ci_how_many_hours_should_they_stay"
                        value="{{ $ci_order_item->how_many_hours_should_they_stay }}">
                    <input type="hidden" name="ci_weekly_off_charge" id="ci_weekly_off_charge" value="0">
                    <input type="hidden" name="ci_subservice_id" id="ci_subservice_id"
                        value="{{ $ci_order_item->subservice_id }}">
                    ...
                    <div class="tab2" id="tab2-div">
                            ...
                            <div id="cleaner_section">
                                @if ($ci_orders->user_id != '')
                                    ...
                                    <div class="form-group mb-3">
                                        <label class="form-label fw500 dark-color" for="country">Select your preferred
                                            cleaner</label>
                                                ...
                                                <ul class="splide__list">
                                                    @foreach ($cleaners as $data)
                                                        <li class="splide__slide text-center ">
                                                            <label for="cleaner_{{ $data->id }}"
                                                                class="cleaner-label-wrapper">
                                                                <input type="radio" id="cleaner_{{ $data->id }}"
                                                                    name="cleaner" class="cleaners-radio"
                                                                    value="{{ $data->name }}"
                                                                    {{ isset($ci_order_item->cleaner_id) && $ci_order_item->cleaner_id == $data->id ? 'checked' : '' }}
                                                                    hidden>
                                                                ...
                                                                <div class="cleaners-div"
                                                                    ...
                                                                    <div class="items">
                                                                        ...
                                                                        <div class="cleaner-detail">
                                                                            ...
                                                                            @else
                                                                                <p>&nbsp;</p>
                                                                                <p class="cleaners-para">Best in Your
                                                                                    Area</p>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                        ...
                        <div class="form-group mb-3">
                            <label class="form-label fw500 dark-color " for="country">When would you like your
                                service?</label>
                        ...
                        <div class="form-group mb-3">
                            <label class="form-label fw500 dark-color" for="country">What time would you like us to
                                start?</label>
                                    ...
                                    @if ($timeslot_service && $timeslot_service->is_active == 1)
                                        <div class="surcharge-badge-timeslot items">
                                            ...
                                            <input type="radio" id="time{{ $i }}" name="time_slot"
                                                ...
                                                {{ isset($ci_order_item->time_slot) && $ci_order_item->time_slot == $timeslot_data->id ? 'checked' : '' }}>
                        ...
                        <button type="button"
                            ...
                            id="nextBtn12" onclick="reschedule_order();">Reschedule</button>
...
@if ($ci_order_item->subservice_id == 28)
    @if ($ci_orders->user_id != '')
        @foreach ($cleaners as $data)
            <div class="modal modal-mobile-bottom cleaners-modal" id="cleaner_description_{{ $data->id }}"
                tabindex="-1" aria-labelledby="materialTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-bottom modal-lg modal-dialog-centered" id="modal-digi"
...
<script>
    ...
    ${
        ...
        ? `<div class="surcharge-badge-dayslot" style="position:relative;">
               <span>+ <p class="currency_dhiram"></p> 5</span>
           </div>`
        : ''
    }
`;
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                allSlots.forEach(function(slot, index) {
                    ...
                    if (price > 0) {
                        slotHtml += `<span>+ <p class="currency_dhiram"></p> ${price}</span>`;
                    }
                    ...
                    booked_slots.forEach(function(slot, index) {
                        ...
                        slots.each(function(i) {
                            if ($(this).val() === slotValue) {
                                ...
                                $(this).closest('.surcharge-badge-timeslot').addClass(
                                    'disabled-slot');
                                ...
                                for (let j = 1; j <= disableHours; j++) {
                                    ...
                                    if (nextSlot.length) {
                                        ...
                                        nextSlot.closest('.surcharge-badge-timeslot')
