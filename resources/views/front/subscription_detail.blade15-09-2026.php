...
<style>
    ...
    @media (max-width: 576px) {
        ...
        .action-banner-actions .vc-btn-primary {
            flex: 1;
            width: auto;
        }
    }
    ...
    .manage-sub-btn {
        ...
        border-radius: 8px;
        box-shadow: 0 2px 4px rgba(0, 64, 230, 0.1);
        transition: all 0.2s;
    }

    .manage-sub-btn:hover {
        background-color: #f8fafc !important;
        box-shadow: 0 4px 6px rgba(0, 64, 230, 0.15);
                ...
                <div class="col-lg-8">
                    ...
                    <div class="detail-card">
                        ...
                        <div class="quick-actions">
                            {{-- <div class="qa-btn" onclick="openRescheduleModal(null, '{{ $subscription->next_visit_date }}', '{{ $subscription->next_visit_time }}')"><i class="far fa-calendar-alt" style="margin-right:5px;"></i> Reschedule
                        </div>
...
<script>
    ...
    function confirmReschedule() {
        ...
        fetch('{{ route('
            ...
            .then(data => {
                if (data.status == 1) {
                    let newDate = new Date(newDateInput).toLocaleDateString('en-GB', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric'
                    });
    ...
    function submitAddressChangeRequest() {
        ...
        fetch('{{ route('
                subscription.request_address_change ') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
